...
As part of our proposed first contribution, we will attempt to create automatic methods that can map gazes to objects more accurately than using this na\"{i}ve approach. Tentative plans for such methods are described below in section 3.1.1. We will test, and validate these methods, and the feasibility of the DOI approach to produce accurate data in general, by: (i) using the methods to instrument a concrete, representative visualization (described in section 3.1.3); (ii) gathering DOI data from human subjects using the visualization; and (iii) comparing the collected data to a ground-truth derived as discussed in section 3.1.2. 
